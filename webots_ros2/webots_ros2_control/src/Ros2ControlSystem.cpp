...
namespace webots_ros2_control
{
  void Ros2ControlSystem::init(webots_ros2_driver::WebotsNode *node, const hardware_interface::HardwareInfo &info)
  {
    ...
    /// wsh_annotation
    _time_step = node->robot()->getBasicTimeStep();
    ...
    for (hardware_interface::ComponentInfo component : info.joints)
    {
      ...
      /// wsh_annotation
      joint.position_old = joint.sensor->getValue();
  ...
  std::vector<hardware_interface::StateInterface> Ros2ControlSystem::export_state_interfaces()
  {
    ...
    for (Joint &joint : mJoints)
      if (joint.sensor)
      {
        ...
        interfaces.emplace_back(hardware_interface::StateInterface(joint.name, hardware_interface::HW_IF_EFFORT, &(joint.effort)));
      }
  ...
  hardware_interface::return_type Ros2ControlSystem::read()
  {
    for (Joint &joint : mJoints)
    {
      if (joint.sensor)
      {
        ...
        /// wsh_annotation
        joint.velocity = (joint.position - joint.position_old) * 1000 / _time_step; // _time_step->unit: ms
        joint.position_old = joint.position;
        joint.effort = joint.effortCommand;
        // std::cout << "****************************************************" << _time_step << std::endl;
      }
    }
